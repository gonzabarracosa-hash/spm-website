// Scene components for the ACOTARC explainer video — SPM brand system.
...
const DEMO_VIDEO_SRC = './uploads/Recording 2026-07-28 091316.mp4';
...
function ShellDrawing({ theme, accent, dimensioned }) {
  const bosses = [
    { x: 90, y: 40, deg: 0 }, { x: 220, y: 60, deg: 45 }, { x: 340, y: 130, deg: 90 },
    { x: 260, y: 240, deg: 150 }, { x: 120, y: 250, deg: 220 }, { x: 40, y: 150, deg: 300 },
  ];
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
      <div style={{ position: 'relative', width: 420, height: 300, border: `1px solid ${theme.panelBorder}`, borderRadius: 6, background: theme.chrome }}>
        {bosses.map((b, i) => (
          <div key={i} style={{ position: 'absolute', left: b.x, top: b.y }}>
            <div style={{ width: 16, height: 16, borderRadius: '50%', border: `2px solid ${theme.textMuted}`, background: theme.panel }} />
            {dimensioned ? (
              <div style={{ position: 'absolute', top: 20, left: -10, fontFamily: BODY, fontSize: 10, color: accent, whiteSpace: 'nowrap', fontWeight: 600 }}>
                {(b.x * 5)} ({b.deg}°)
              </div>
            ) : null}
          </div>
        ...
        {dimensioned ? (
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: -22, borderTop: `1.5px solid ${accent}` }} />
        ) : null}
      </div>
      <div style={{ fontFamily: BODY, fontSize: 16, fontWeight: 500, color: dimensioned ? accent : theme.textMuted }}>{dimensioned ? 'Shell_Develop.dwg — dimensioned' : 'Shell_Develop.dwg'}</div>
    </div>
  );
}
...
function Hook({ localTime }) {
  ...
  const items = ['Locate every boss and hanger on the development', 'Convert linear position to degrees by hand', 'Type each ordinate dimension one by one', 'Keep every text aligned on the same axis'];
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 160px' }}>
      <div style={{ opacity: headOp, transform: `translateY(${(1 - headOp) * 16}px)` }}>
        <div style={{ fontFamily: HEAD, fontSize: 60, fontWeight: 800, color: th.text, lineHeight: 1.15, maxWidth: 1200 }}>Dimensioning bosses and hangers by hand takes hours.</div>
      </div>
      <div style={{ opacity: subOp, transform: `translateY(${(1 - subOp) * 12}px)`, marginTop: 22 }}>
        <div style={{ fontSize: 26, color: th.textMuted, fontFamily: BODY }}>Every nozzle on a cylindrical shell, measured and typed by hand.</div>
      </div>
...
function Solution({ localTime }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 28 }}>
      <div style={{ opacity: op, transform: `scale(${0.94 + 0.06 * op})`, padding: '8px 20px', borderRadius: 999, border: `1.5px solid ${th.accent}`, background: `${th.accent}14`, fontFamily: BODY, fontSize: 18, fontWeight: 700, color: th.accent, letterSpacing: '0.08em', textTransform: 'uppercase' }}>AutoCAD LISP — ACOTARC</div>
      <div style={{ opacity: op, fontFamily: HEAD, fontSize: 56, fontWeight: 800, color: th.text, textAlign: 'center', maxWidth: 1200 }}>One command dimensions it all.</div>
    </div>
  );
}
...
function Demo({ localTime }) {
  ...
  const captions = [
    { start: 0.6, end: 4.2, text: 'Click the part origin — where the development starts (X=0).' },
    { start: 4.3, end: 7.8, text: 'Click the 0° reference point.' },
    { start: 7.9, end: 11.4, text: 'Click the end of the development — the 360° point.' },
    { start: 11.5, end: 15.0, text: 'Click free space to set the text alignment height.' },
    { start: 15.1, end: 19.0, text: 'Window-select every boss and hanger to dimension.' },
    { start: 19.1, end: 23.0, text: 'ACOTARC draws every ordinate dimension in mm and degrees.' },
  ];
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: `translate(-50%,-50%) scale(${0.96 + 0.04 * winOp})`, opacity: winOp }}>
        <SoftwareWindow theme={th} title="AutoCAD — screen recording (4x speed)">
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 40 }}>
      <div style={{ display: 'flex', gap: 60 }}>
        <div style={{ opacity: leftOp, transform: `scale(${0.94 + 0.06 * leftOp})`, padding: 28, borderRadius: 14, background: th.panel, border: `1px solid ${th.panelBorder}` }}>
          <ShellDrawing theme={th} dimensioned={false} />
        </div>
        <div style={{ opacity: rightOp, transform: `scale(${0.94 + 0.06 * rightOp})`, padding: 28, borderRadius: 14, background: th.panel, border: `1.5px solid ${th.accent}`, boxShadow: `0 0 0 6px ${th.accent}18` }}>
          <ShellDrawing theme={th} accent={th.accent} dimensioned={true} />
        </div>
      </div>
      <div style={{ opacity: statusOp, display: 'flex', alignItems: 'center', gap: 14 }}>
        <span style={{ color: SUCCESS, fontSize: 24, fontWeight: 700 }}>✓</span>
        <div style={{ fontFamily: BODY, fontSize: 24, fontWeight: 500, color: th.text }}>Every boss and hanger dimensioned in mm and degrees.</div>
      </div>
    </div>
  );
}
...
function Benefits({ localTime }) {
  ...
  const items = ['No manual angle math', 'No overlapping dimension text', 'Done in seconds, not hours'];
  return (
...
function Outro({ localTime }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, textAlign: 'center', padding: '0 200px' }}>
      <div style={{ opacity: headOp, transform: `translateY(${(1 - headOp) * 14}px)`, fontFamily: HEAD, fontSize: 56, fontWeight: 800, color: th.text, lineHeight: 1.2 }}>Automate cylindrical dimensioning with ACOTARC.</div>
      <div style={{ opacity: subOp, transform: `translateY(${(1 - subOp) * 10}px)`, fontFamily: BODY, fontSize: 20, fontWeight: 700, color: th.accent, letterSpacing: '0.1em', textTransform: 'uppercase' }}>One command. Zero overlaps.</div>
    </div>
  );
}
...
Object.assign(window, { AcotarcExplainer });
